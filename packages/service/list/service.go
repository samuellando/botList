...
type ListStore interface {
	...
	Update(list.ItemList, list.ItemList) (list.ItemList, error)
...
func (ls ListService) Update(w http.ResponseWriter, act action.Update) {
	obj := act.Object()
	target := act.TargetCollection()
	...
	_, err = ls.store.Update(target, obj)
	if err != nil {
		panic(err)
	}
	res := act.WithResult(result.Create("201", "Updated"))
	jsonB := jsonld.Marshal(util.Sign(res, pk))
	ls.messageQueue <- jsonB
	w.WriteHeader(202)
	...
	for _, h := range l.Hooks() {
		switch ah := h.(type) {
		case hook.ActionHook:
			if !slices.Contains(ah.OnActionType(), "Update") {
				continue
			}
			ea := action.CreateExecute(func(ev *action.ExecuteValues) {
				ev.Agent = l
