...
func (a Execute) MarshalJSON() ([]byte, error) {
	type External struct {
		...
		TargetRunner       list.Runner    `json:"http://fedilist.com/targetRunner"`
...
func (a Execute) TargetRunner() list.Runner {
...
func parseExecute(json map[string]any) (Execute, error) {
	...
	var runner list.Runner
	if json, ok := objs["targetRunner"]; ok {
		runner, err = list.ParseRunner(json)
		if err != nil {
			return Execute{}, err
		}
	} else {
